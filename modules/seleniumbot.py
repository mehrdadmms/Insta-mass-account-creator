...
class AccountCreator:
    ...
    def creation_config(self):
        try:
            if self.use_local_ip_address is False:
                if self.use_custom_proxy is False:
                    for i in range(0, config.Config['amount_of_account']):
                        if len(self.sockets) > 0:
                            current_socket = self.sockets.pop(0)
                            try:
                                self.create_account(current_socket)
                            except Exception as e:
                    ...
                    with open(config.Config['proxy_file_path'], 'r') as file:
                        ...
                        for proxy in content:
                            ...
                            if amount_per_proxy != 0:
                                ...
                                for _ in range(0, amount_per_proxy):
                                    try:
                ...
                for _ in range(0, config.Config['amount_of_account']):
                    try:
                        self.create_account()
                    except Exception as e:
